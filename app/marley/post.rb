...
module Marley
  ...
  # = Articles
  # Data source is Marley::Application::DATA_DIRECTORY (set in <tt>config.yml</tt>)
  class Post
    ...
    def self.load_directories_with_posts(options={})
      if options[:draft]
        Dir[File.join(Application::DATA_DIRECTORY, '*')].select { |dir| File.directory?(dir)  }.sort
      else
        Dir[File.join(Application::DATA_DIRECTORY, '*')].select { |dir| File.directory?(dir) and not dir.include?('.draft')  }.sort
      end
    end
